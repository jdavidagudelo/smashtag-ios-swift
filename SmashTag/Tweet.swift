            ...
            if let startIndex = (indices?.firstObject as? NSNumber)?.integerValue {
                if let endIndex = (indices?.lastObject as? NSNumber)?.integerValue {
                    ...
                    if length > 0 {
                        ...
                        if end > start {
                            let startRange = inText.startIndex.advancedBy(start)
                            let endRange = inText.startIndex.advancedBy(end-1)
                            range = startRange...endRange
                            keyword = inText.substringWithRange(range)
                            if prefix != nil && !keyword.hasPrefix(prefix!) && start > 0 {
                                let startPrefix = inText.startIndex.advancedBy(start-1)
                                let endPrefix = inText.startIndex.advancedBy(end-2)
